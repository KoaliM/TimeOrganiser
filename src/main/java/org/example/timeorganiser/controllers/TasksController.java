...
public class TasksController {
    ...
    public ResponseEntity<List<Map<String, Object>>> getAll() {
        return ResponseEntity.ok(IntegrationData.tasks());
    }

    @PostMapping
    public ResponseEntity<Map<String, Object>> create(@RequestBody Map<String, Object> task) {
        IntegrationData.tasks().add(task);
    ...
    public ResponseEntity<Void> delete(@PathVariable String title) {
        IntegrationData.tasks().removeIf(item -> title.equalsIgnoreCase(String.valueOf(item.get("title"))));
        return ResponseEntity.noContent().build();
    }

    private ResponseEntity<Map<String, Object>> updateByTitle(String title, Map<String, Object> body) {
        return IntegrationData.tasks().stream()
